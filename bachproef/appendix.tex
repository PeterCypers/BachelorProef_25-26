...
%t'do: move many code fragments here (Ollama section)
...
\label{sec:ollamalokaalofflineresultaten}
...
\label{sec:ollamacloudsigninonlineresultaten}
...
\label{sec:ollamacloudapionlineresultaten}
...
\label{sec:ollamaasyncclientofflineresultaten}
...
\label{sec:ollamaasyncgeneratorofflineresultaten}
...
\label{sec:ollamaimgasyncgeneratorofflineresultaten}
...
\label{sec:ollamalocalhostapiimgofflineresultaten}
...
\label{sec:ollamaplustoolsofflineresultaten}

Zie: \ref{sssec:ollamaplustools}

\section{Ollama + JavaScript (offline) resultaten}
\label{sec:ollamaplusjavascriptofflineresultaten}
...
\section{LangChain}
\label{sec:langchain2}

\subsection{LangChain Eerste Code Voorbeeld}
\label{ssec:langchaineerstecodevoorbeeld}
...
\label{ssec:langchainverschusecasecodevb}

\subsubsection{Code Generatie:}
\label{sssec:codegeneratie}
...
\textbf{Output:} (Figuur~\ref{fig:lcelcodegenlangchainagent})
\begin{minted}{markdown}
...
\textbf{Output:} (Figuur~\ref{fig:lceltextsummarylangchainagent})
...
\label{sssec:tekstclassificatie}
...
\textbf{Output:} (Figuur~\ref{fig:lceltextclassificationlangchainagent})
\begin{tcolorbox}[colback=gray!20, colframe=gray!80]
Music \\
Reasoning Skill: \\
...
\subsection{LangChain Document-Loaders \& Retrievers}
\label{ssec:langchaindocumentloadersandretrievers}

In volgend demo's is een poging ondernomen om met het ophalen van documenten te werken met LangChain.

\subsubsection{Document-Loaders}
\label{sssec:documentloaders}
% langchain_document_loaders.png

Er wordt een initiële poging gedaan om de Open Api Specification (OAS) als document in te laden en met een langchain\_community package dat toelaat informatie vanop een website op te halen. De structuur van dit ingeladen document wordt vervolgens onder de loep genomen (Figuur~\ref{fig:langchaindocumentloaders}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{langchain_document_loaders.png}
    \caption[LangChain Document-Loaders]{\label{fig:langchaindocumentloaders}LangChain Document-Loaders}
\end{figure}

Dit was een initiële test om te zien of het lukte om een document van het internet in te laden, gebruikmakend van langchain\_community.document\_loaders. Langchain community is iets wat nog buiten langchain\_core staat en als extensie van langchain kan gezien worden. In dit codefragment wordt de API-key niet gebruikt, het is enkel een korte test om te zien hoe het binnengehaalde document eruit ziet, er waren een aantal warnings over het vriendelijk omgaan met API-calls. Er wordt verwacht dat de caller zich kenbaar maakt. De metadata en eerste 100 tekens worden uitgeprint in de terminal. Wat in het codefragment in comment staat aan het einde van de file zijn alle fields binnen het eerste docs object. Een document object bevat dus heel wat methodes en attributen. (In volgende output werden een 50tal lege lijnen verwijderd) \\


\textbf{Output:}
\begin{tcolorbox}[colback=gray!20, colframe=gray!80]
    \begin{verbatim}
C:\Users\peter\AppData\Local\Programs\Python\Python312\Lib\
site-packages\urllib3\connectionpool.py:1097: 
InsecureRequestWarning: Unverified HTTPS request is being made to
host 'swagger.io'. 
Adding certificate verification is strongly advised. See:
https://urllib3.readthedocs.io/en/latest/advanced-usage.html#tls-
warnings
warnings.warn(
{'source': 'https://swagger.io/specification/', 
'title': '\r\n\tOpenAPI 
Specification - Version 3.1.0 | Swagger\r\n',
'description': 'The OpenAPI Specification defines a 
standard interface to RESTful APIs which allows both humans and 
computers to understand service capabilities without 
access to source code, documentation, or network 
traffic inspection.', 'language': 'No language found.'}
...
Dit werkte nog redelijk goed, maar er is gekozen eerst verder te gaan naar het onderzoeken van een volgende technologie.


\subsubsection{Retrievers}
\label{sssec:retrievers}
%langchain_retrievers.png

Hier zien we een mogelijke uitwerking van retrievers (Figuur~\ref{fig:langchainretrievers}), gelijkaardig aan RAG in het opzicht dat er uit een opslag van data gezocht wordt naar context relevant aan de query, maar hier is het niet ophalen van relevante data uit eigen bronnen, maar bepaalde bronnen op het internet. Deze demo maakt ook nog gebruik van LCEL (LangChain expression language). Maar zoals in de comment-blokken al laat doorschijnen werkten deze demo's niet, er zijn 2 verschillende retrievers van info, WikipediaRetriever en AmazonKendraRetriever. Voor beide waren er credentials nodig die men niet perse wou vrijmaken om deze demo uit te werken. Er gaat namelijk echt geld ter verspilling bij het gebruiken van LLM-API-calls. Er werden wel OPENAI\_API\_KEY en LANGSMITH\_API\_KEY in de environment ingeladen om alsnog te proberen. LangSmith is ook nog verbonden aan LangChain. Voorlopig zijn er geen plannen om dit verder te onderzoeken.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{langchain_retrievers.png}
    \caption[LangChain Retrievers]{\label{fig:langchainretrievers}LangChain Retrievers}
\end{figure}

\section{LlamaIndex}
\label{sec:llamaindex2}

\subsection{Multi-Agent Workflow met LlamaIndex}
\label{ssec:multiagentworkflowmetllamaindex}
...
Volgende demo maakt gebruik van LlamaIndex \ref{sec:llamaindex} en is een voorbeeld van een multi-agent workflow. Er worden tools gebruikt, API-calls gemaakt en agents kunnen beslissingen maken zoals het wel of niet overhandigen van een afgewerkte taak aan een volgende agent (handoff). AgentWorkflow maakt het mogelijk om meerdere agents die ook over hun eigen tools beschikken te laten samenwerken. Workflows kunnen ook werken wanneer men maar 1 agent meegeeft. De Tavily web-zoek API wordt gebruikt in deze demo met eigen API-key. \\

Deze demo maakt ook gebruik van het Generator $\rightarrow$ Evaluator (ook wel Generator $\rightarrow$ Optimizer) patroon, zie Sectie: Evaluator-Optimizer Pattern (\ref{sssec:evaluatoroptimizerpattern}). De normale uitvoer staat in de main() functie in commentaar en de uitgebreide informatie wordt uitgeprint in de terminal. Vervolgens bekijken we de demo-code en uitleg bij de code (Figuur~\ref{fig:llamaindexmultiagentworkflow1}; Figuur~\ref{fig:llamaindexmultiagentworkflow2}; Figuur~\ref{fig:llamaindexmultiagentworkflow3}; Figuur~\ref{fig:llamaindexmultiagentoutput1}; Figuur~\ref{fig:llamaindexmultiagentoutput2}):

% dit is ook een voorbeeld van: generator -> evaluator pipeline -> my research - write and review agents

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{llamaindex_multi_handoff_01.png}
    \caption[LlamaIndex multi-agent workflow deel 1]{\label{fig:llamaindexmultiagentworkflow1}LlamaIndex multi-agent workflow deel 1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{llamaindex_multi_handoff_02.png}
    \caption[LlamaIndex multi-agent workflow deel 2]{\label{fig:llamaindexmultiagentworkflow2}LlamaIndex multi-agent workflow deel 2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{llamaindex_multi_handoff_03.png}
    \caption[LlamaIndex multi-agent workflow deel 3]{\label{fig:llamaindexmultiagentworkflow3}LlamaIndex multi-agent workflow deel 3}
\end{figure}

\textbf{Uitleg bij de code:} API-keys worden klaargezet, de Tavily web-zoek API wordt gebruikt in deze demo. De gebruikte tools worden als asynchrone functies gedefinieerd, met elk een interne docstring dat de agents helpt bepalen of de tool nodig zal zijn voor het afhandelen van een gekregen taak. Een workflow context object is beschikbaar als parameter bij de verschillende tools dit maakt het mogelijk voor de agents om van een gedeelde state te lezen en schrijven dat enkel beschikbaar is tijdens deze workflow. \\

De agents worden gedefinieerd, ze krijgen een naam, systeem prompt, LLM en tools. Deze krijgen ook een description (omschrijving), dit is vooral nodig voor agent-orchestratie binnen de workflow, dit kan de workflow helpen bepalen naar welke agent de taak gestuurd (ge-rout) wordt. Can\_handoff\_to is een lijst van agents waaraan de output van een agent kan overhandigd worden, merk op dat de write\_agent kan kiezen tussen 2 verschillende agents. De keuze om output naar een volgende agent te sturen ligt deels bij de agent en deels bij de workflow (A:can\_handoff\_to \& WF: description + routing). \\

Vervolgens wordt de AgentWorkflow gedefinieerd dat de verzameling van agents bevat, een root\_agent dat als eerste aan het werk zal gaan alsook de state dat zal aangepast worden door de verschillende agents. Uiteindelijk zal bij aanroep van deze workflow de interne werking uitgeprint worden, afhankelijk van type event, waarbij deze demo inzicht geeft over de interne werking van de agents. Het deel code dat in commentaar staat is de normale output die nu niet zal getoond worden. (multi-agent workflows kunnen mogelijks veel tokens gebruiken) \\

\textbf{Output:}
% llama-multi-output-1.png
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{llama-multi-output-1.png}
    \caption[LlamaIndex multi-agent workflow deel 3]{\label{fig:llamaindexmultiagentoutput1}LlamaIndex multi-agent output deel 1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{llama-multi-output-2.png}
    \caption[LlamaIndex multi-agent workflow deel 3]{\label{fig:llamaindexmultiagentoutput2}LlamaIndex multi-agent output deel 2}
\end{figure}

\textbf{Output uitleg:} We zien de opgehaalde informatie, wat er in de notes als state bewaard werd en naar welke agent de handoff zal gebeuren, de output eindigt hier wel, dus de write\_agent en review\_agent kwamen niet aan bod, mogelijks door een timeout of onafgehandelde fout in een van de asynchrone bewerkingen.

\section{Test Generatie met Github Copilot}
\label{sec:testgeneratiemetgithubcopilot}
...
\begin{minted}{php}
    ...
    class CategoryResourceTest extends TestCase
    {
        ...
        /**
        * Test that CategoryResource has correct model assigned
        */
        public function test_resource_uses_category_model(): void
        {
            $this->assertEquals(Category::class, CategoryResource::$model);
        }
        
        /**
        * Test that navigation icon is properly set
        */
        public function test_navigation_icon_is_set(): void
        {
            $this->assertEquals('heroicon-o-folder', CategoryResource::$navigationIcon);
        }
        
        /**
        * Test that authorization is skipped
        */
        public function test_authorization_is_skipped(): void
        {
            $this->assertTrue(CategoryResource::$shouldSkipAuthorization);
        }
        
        /**
        * Test form schema contains required fields
        */
        public function test_form_has_name_and_slug_inputs(): void
        {
            $form = CategoryResource::form(\Filament\Forms\Form::make());
            $schema = $form->getComponents();
            
            $this->assertNotEmpty($schema, 'Form schema should not be empty');
            
            $fieldNames = array_map(fn($field) => $field->getName(), $schema);
            $this->assertContains('name', $fieldNames, 'Form should contain name field');
            $this->assertContains('slug', $fieldNames, 'Form should contain slug field');
        }
        ...
        */
        public function test_name_field_is_required(): void
        {
            ...
            $this->assertTrue($nameField->isRequired(), 'Name field should be required');
        }
        
        /**
        * Test that slug field is required
        */
        public function test_slug_field_is_required(): void
        {
            $form = CategoryResource::form(\Filament\Forms\Form::make());
            $slugField = $form->getComponent('slug');
            
            $this->assertInstanceOf(TextInput::class, $slugField);
            $this->assertTrue($slugField->isRequired(), 'Slug field should be required');
        }
        
        /**
        * Test slug generation on create operation
        */
        public function test_slug_is_generated_from_name_on_create(): void
        {
            $testName = 'Test Category';
            $expectedSlug = Str::slug($testName);
            
            $form = CategoryResource::form(\Filament\Forms\Form::make());
            $nameField = $form->getComponent('name');
            
            $this->assertNotNull($nameField->getAfterStateUpdated());
        }
        
        /**
        * Test slug is not updated on edit operation
        */
        public function test_slug_is_not_regenerated_on_edit(): void
        {
            $category = Category::factory()->create(['slug' => 'original-slug']);
            
            // Simulate edit operation - slug should remain unchanged
            $originalSlug = $category->slug;
            $category->update(['name' => 'Updated Name']);
            
            // In edit mode, slug generation is skipped
        ...
        */
        public function test_table_has_name_and_slug_columns(): void
        {
            $table = CategoryResource::table(\Filament\Tables\Table::make());
            $columns = $table->getColumns();
            
            $columnNames = array_map(fn($column) => $column->getName(), $columns);
            $this->assertContains('name', $columnNames, 'Table should display name column');
            $this->assertContains('slug', $columnNames, 'Table should display slug column');
        }
        
        /**
        * Test table has edit action
        */
        public function test_table_has_edit_action(): void
        {
            ...
            $hasEditAction = collect($actions)->some(
            fn($action) => $action instanceof \Filament\Tables\Actions\EditAction
            );
            
            $this->assertTrue($hasEditAction, 'Table should have edit action');
        }
        
        /**
        * Test table has delete bulk action
        */
        public function test_table_has_delete_bulk_action(): void
        {
            $table = CategoryResource::table(\Filament\Tables\Table::make());
            $bulkActions = $table->getBulkActions();
            
            $this->assertNotEmpty($bulkActions, 'Table should have bulk actions');
        }
        
        /**
        * Test resource has correct relations
        */
        public function test_resource_has_posts_relation_manager(): void
        {
            $relations = CategoryResource::getRelations();
            
            $this->assertContains(PostsRelationManager::class, $relations);
        }
        
        /**
        * Test resource has correct pages
        */
        ...
        */
        public function test_index_page_has_correct_route(): void
        {
            $pages = CategoryResource::getPages();
            
            $this->assertStringContainsString('/', $pages['index']);
        }
        
        /**
        * Test create page route
        */
        public function test_create_page_has_correct_route(): void
        {
            $pages = CategoryResource::getPages();
            
            $this->assertStringContainsString('/create', $pages['create']);
        }
        
        /**
        * Test edit page route with parameter
        */
        public function test_edit_page_has_correct_route_with_parameter(): void
        {
            $pages = CategoryResource::getPages();
            
            $this->assertStringContainsString('{record}/edit', $pages['edit']);
        }
    }
\end{minted}
