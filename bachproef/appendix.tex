...
Er wordt een initiele poging gedaan om de Open Api Specification (OAS) als document in te laden en met een langchain\_community package dat toelaat informatie vanop een website op te halen. De structuur van deze ingeladen document wordt vervolgens onder de loep genomen (Figuur~\ref{fig:langchaindocumentloaders}).
...
Dit was een initiële test om te zien of het lukte om een document van het internet in te laden, gebruikmakend van langchain\_community.document\_loaders. Langchain community is iets wat nog buiten langchain\_core staat en als extensie van langchain kan gezien worden. In dit codefragment wordt de API-key niet gebruikt, het is enkel een korte test om te zien hoe de binnengehaald document eruit ziet, er waren een aantal warnings over het vriendelijk omgaan met API-calls. Er wordt verwacht dat de caller zich kenbaar maakt. De metadata en eerste 100 tekens worden uitgeprint in de terminal. Wat in de codefragment in comment staat aan het einde van de file zijn alle fields binnen het eerste docs object. Een document object bevat dus heel wat methodes en attributen. (In volgend output werden ~50tal lege lijnen verwijderd) \\
...
Volgend demo maakt gebruik van LlamaIndex \ref{sec:llamaindex} en is een voorbeeld van een multi-agent workflow. Er worden tools gebruikt, API-calls gemaakt en agents kunnen beslissingen maken zoals het wel of niet overhandigen van een afgewerkte taak aan een volgend agent (handoff). AgentWorkflow maakt het mogelijk om meerdere agents die ook over hun eigen tools beschikken te laten samenwerken. Workflows kunnen ook werken wanneer men maar 1 agent meegeeft. De Tavily web-zoek API wordt gebruikt in deze demo met eigen API-key. \\
...
\textbf{Uitleg bij de code:} API-keys worden klaargezet, de Tavily web-zoek API wordt gebruikt in deze demo. De gebruikte tools worden als asynchrone functies gedefinieerd, met elk een interne docstring dat de agents helpt bepalen of de tool nodig zal zijn voor het afhandelen van een gekregen taak. Een workflow context object is beschikbaar als parameter bij de verschillende tools dit maakt het mogelijk voor de agents om van een gedeeld state te lezen en schrijven dat enkel beschikbaar is tijdens deze workflow. \\
