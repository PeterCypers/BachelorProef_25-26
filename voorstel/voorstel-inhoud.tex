...
De \textbf{onderzoeksvraag} vanuit IT1 luidt als volgt: 
\begin{tcolorbox}[colback=yellow!30]
    \emph{Hoe kan een AI agent bijdragen aan het ondersteunen van het ontwikkel- en testproces van RESTful APIs binnen een agile softwareontwikkeltraject?} 
\end{tcolorbox}
Hieronder is het probleem opgesplitst in een aantal \textbf{deelvragen} die zullen bijdragen tot een uitwerking van de hoofdonderzoeksvraag:

\begin{itemize}
  \item[] \begin{tcolorbox}[colback=yellow!30]
      Wat zijn de huidige uitdagingen bij het ontwikkelen en testen van RESTful APIs?
    \end{tcolorbox} 
  \item[] \begin{tcolorbox}[colback=yellow!30]
      Welke taken binnen dit proces zijn het meest repetitief of foutgevoelig?
    \end{tcolorbox}
  \item[] \begin{tcolorbox}[colback=yellow!30]
      Welke bestaande tools en frameworks worden momenteel gebruikt voor API testing en documentatie?
    \end{tcolorbox}
  \item[] \begin{tcolorbox}[colback=yellow!30]
      Welke AI-technieken (bv. LLMs, NLP, code generation) zijn geschikt om deze taken te ondersteunen?
    \end{tcolorbox}
  \item[] \begin{tcolorbox}[colback=yellow!30]
      Hoe kan een AI agent geïntegreerd worden in een bestaande ontwikkelworkflow?
    \end{tcolorbox}
  \item[] \begin{tcolorbox}[colback=yellow!30]
      Wat zijn de beperkingen en risico’s van het inzetten van een AI agent in dit domein?
    \end{tcolorbox}
\end{itemize}
...
\subsection{Wat zijn REST API's?}
...
\begin{enumerate}
    ...
    Client-server architectuur. Separation of concerns is het principe achter deze constraint. Het scheiden van user interface (client) en data storage (server) zorgt voor verbeterde portability en scalability. Zo kunnen de twee onafhankelijk van elkaar evolueren.
    ...
    De hoofdzaak bij REST, het zijn deze principes die zorgen voor een uniforme interactie (interface) tussen netwerk-componenten (vb. Browser $\rightarrow$ Servers/gateways). Er zijn 4 architecturele beperkingen nodig om het gedrag van componenten vast te leggen:
    ...
    We verbeteren de schaalbaarheid door van de API een gelaagd systeem te maken dat bestaat uit hiërarchisch lagen van componenten of services, elke component "ziet" niet verder dan de lagen waarmee die direct in contact is. Er kunnen gemakkelijk lagen toegevoegd of verwijderd worden en elke laag heeft zijn eigen verantwoordelijkheid, daardoor krijgen we een separation of concerns. Dit maak ook de encapsulatie van verouderde(legacy) services mogelijk zodat de oude en nieuwe services naast elkaar in werking kunnen blijven.
...
Wanneer een API aan alle verplichte constraints voldoet wordt het RESTful genoemd. Als het ontwerp van een API de meeste REST constraints volgt maar toch minstens 1 verplicht constraint schendt kan het hooguit "REST-like" genoemd worden, het niet toepassen van HATEOAS is een mogelijke oorzaak hiervan \autocite{Bush2020}.
...
De OAS is meer dan een guideline, het is zowat dé conventie die gehanteerd wordt bij het ontwikkelen van APIs. Deze omschrijft onder andere wat de correcte structuur is van API-requests \& responses. Het is belangrijk dat één werkwijze wordt gevolgd door ontwikkelaars zodat de werking van APIs voorspelbaar en gebruiksvriendelijk blijft. Deze volgt REST-principes maar wordt ook up-to-date gehouden door de OpenAPI Initiative onder de Linux Foundation. \autocite{OpenAPIInitiativea}
\begin{quotation}
...
Over de concepten van Artificiële Intelligentie (AI) en Large Language Models (LLMs) zal er nu nog niet verder worden uitgeweid. Eerst een toespitsing op Agents.

Toch even LLMs in het kort: Een gesimplificeerde omschrijving zou zijn dat een \textbf{LLM} een zeer sterke autocomplete machine is die een bepaalde hoeveelheid trainings-data verwerkt heeft om op een input te kunnen reageren gebruik makend van dat uitgebreide arsenaal aan informatie en tracht een zo nauwkeurig mogelijk antwoord als een puzzel samen te stellen en dan terug te sturen als response.
...
\begin{enumerate}
    \item \textbf{Vertical Architecture:} In deze architectuur heb je één agent die zich als leider gedraagt terwijl de anderen rechtstreeks aan deze agent rapporteren. Er bestaat een duidelijk onderscheid tussen de lead-agent en de agents die eronder staan.
...
\subsubsection{MCP}
Model Context Protocol (MCP) is een open-source standaard om AI applicaties te verbinden met externe systemen (zoals tools of file systemen) \autocite{AnthropicMCP2025}.

Uit een korte video introductie van \textcite{IBMTech2025} zien we dat MCP een transportlaag is tussen een MCP host (vb.: chat app of IDE) en een MCP server. De MCP server is verantwoordelijk voor het verbinden met externe resources/tools, de MCP host krijgt een gebruiker prompt en gaat hiermee naar de MCP server die zal bepalen welke tools beschikbaar zijn en stuurt deze terug naar de host, deze stuurt dan de prompt + beschikbare tools naar een LLM. De LLM zal bepalen welke tool(s) nodig zijn om de prompt te verwerken. Nu kan de MCP host terug naar de MCP server gaan om een tool-result op te halen. De externe resources waarmee de MCP server verbinding maakt kan bijvoorbeeld een database zijn, een file of een API. De MCP host en LLM kunnen deze prompt nu afwerken met de gekregen tool(s).
\subsubsection{RAG}
Vóór Anthropic MCP introduceerden hadden we RAG (Retrieval-Augmented Generation) van Facebook.

Uit het werk - \textit{Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks} - lezen we dat een prompt naar een LLM ondersteund kan worden door het toevoegen van lokale files als context. Ze gebruiken RAG-modellen die een inputsequentie \(x\) gebruiken om tekst documenten \(z\) op te halen die ze gebruiken als bijkomende context voor het genereren van de output sequentie \(y\) \autocite{Lewis2021}.
...
\begin{enumerate}
    ...
    \item Het machine learning model gebruikt zijn NLP skills (content generation, question answering, sentiment analysis, text generation of text summarization) om een response terug naar de API te sturen.
...
LLMs verwerken prompts door de gebruiker input (en eventueel bijkomende context) in tokens op te splitsen, die worden door het model verwerkt om een toepasselijke response terug te sturen. Het doorsturen van tokens naar LLM APIs komt met een bepaald prijskaartje.
...
In de initiële fase zal een marktonderzoek worden gedaan wat betreft reeds beschikbare AI agents en agent-creatie/integratie tools, er zal worden geëxperimenteerd met lokale LLMs die beschikbaar zijn met Ollama\footnote{\href{https://ollama.com/}{https://ollama.com/}}. Deze zullen vergeleken worden met andere mogelijke integraties van LLMs.

\subsection{Integreren van een AI agent}

Voor het maken van de POC AI agent wordt er in de daaropvolgende fase onderzocht welke AI modellen kunnen gebruikt worden, hoe deze geïntegreerd zullen worden in bestaande projecten, de mogelijke hardware restricties, security risico's en kosten die erbij komen kijken. Hier zullen passende agentic design patronen gekozen worden, onder anderen, tool gebruik en lokale context herkenning(vb: toevoegen van specifieke bestanden met boilerplate code aan de context) met behulp van MCP \& RAG. \\

Daarenboven zal ook onderzocht worden in welke ta(a)l(en) en framework(s) de proof of concept zal uitgewerkt worden. \\

Er wordt ook uitgezocht waar de AI agent zal kunnen aangesproken worden, of dit nu in een afzonderlijk bestaand interface zoals één die de gebruiker interacties toelaat in een browser omgeving, of als dit in de standaard-IDE van IT1(binnen de codebase) zal kunnen gebruikt worden als script of extensie.

\subsection{Ondersteuning van de AI agent voor specifieke taken}

Daarna wordt uitgezocht hoe de agent specifiek met het development en testen van APIs bij IT1 kan helpen, er zal getoetst worden wat specifiek bij de opdrachtgever tijdrovende en repetitieve taken zijn bij het API development proces. Hoe de agent zal geïntegreerd worden in reeds gebruikte test/documentatie technieken. Wat zijn de verschillende mogelijkheden dat de agent kan benutten voor het gebruiken en correct toepassen van de OpenAPI Specification(OAS). Hoe zal de AI agent kunnen helpen met het maken van testcases, mogelijks documentatie aanvullen of genereren. Er wordt onderzocht wat eventueel kan geautomatiseerd worden en hoe zal dit gebeuren.
...
\begin{itemize}
    ...
    \item Documentatie aanvullen of genereren
\end{itemize}
...
\subsection{Meten van succes}
Uiteindelijk zal het correct werken van de verschillende functionaliteiten van de agent worden getoetst. Er zal een keuze gemaakt worden of er met methode A \& B of enkel methode A gewerkt wordt.

\subsubsection{AI agent KPI's (A)}

Het meten van success met KPI's. We kiezen bepaalde key performance indicators (KPI's) en meten hoe goed het prototype scoort op de gekozen indicatoren.

Men werkt samen met de AI agent, gebruik makend van de in-house test/documentatie technieken, de agent biedt ondersteuning en/of automatiseert onderdelen van deze technieken. In het geval dat de agent niet gevraagd wordt om zelf tests te genereren kan de agent fouten detecteren en geeft suggesties voor verbeteringen. In dit geval kan de agent getoetst worden op foutdetectie, enkel KPI(2) in tabel \ref{tab:kpis}.

Wanneer de agent wel gevraagd wordt om zelf tests en/of documentatie te genereren wordt deze getoetst aan alle KPIs met uitzondering van KPI(2), zie tabel \ref{tab:kpis}

\subsubsection{Pre‑test / post‑test met controlegroep (B)}

Er kan mogelijks ook gewerkt worden met een controle groep van een vooraf bepaalde aantal API endpoint tests die geschreven zijn zonder het gebruik van de AI agent en een interventie groep waarbij die zelfde tests met AI agent ondersteuning geschreven worden. Hierbij kan de snelheid van het schrijven van de tests, het aantal gemaakte fouten(of hallucinaties), test-coverage en de hoeveelheid zinvolle tests(misschien nog andere factoren) gemeten worden bij beide groepen.
...
Men verwacht dat het prototype zal kunnen aantonen of er een meetbare verbetering is in efficiëntie door het automatiseren of ondersteunen van repetitieve taken bij het ontwikkelen en testen van RESTful APIs. Dat er kan aangetoond worden dat het werken met de AI agent tijdsbesparend is. Ook dat er weinig fouten zullen gebeuren bij het genereren van testcases. Er wordt een hoge test-coverage verwacht. Ook binnen de verwachtingen valt dat de agent zich houdt aan bepaalde best-practices en API-specificaties (OAS).
...
% [t] : top, so table comes first in final page (on the left) vs [htbp]
\begin{sidewaystable}[htbp]
    \centering
    \footnotesize
    \caption[Meetbare KPI's]{\label{tab:kpis}
        Overzicht van de gebruikte KPI's, meet‑technieken en de bijbehorende
        doelwaardes.}
    \begin{tabular}{llll}
        \toprule
        \textbf{KPI} &
        ...
        agent output + manuele controle &
        $\leq5\,\%$ fout‑tolerantie \\[2mm]
        
        Productiviteit &
        tijd per gegenereerde test‑case &
        timestamp vóór/na &
        $\leq30\,\text{s}$ \\[2mm]
        
        Kwaliteit &
        test‑coverage &
        PHPUnit** / Pest** / in‑house testsuite‑coverage‑rapport &
        $\geq80\,\%$ test‑coverage \\[2mm]
        
        Autonomie &
        menselijke interventies &
        vergelijking met minimum test vereisten bij IT1 &
        $\leq$ 2 nodige aanpassingen \\[2mm]
        
        Performantie &
        CPU/GPU‑geheugen‑gebruik &
        Windows‑performance‑tab &
        $\leq60\,\%$ \\[2mm]
        
        Test‑Kwaliteit &
        aantal zinvolle tests per endpoint &
        vergelijking met standaard bij IT1 &
        $\geq$ standaardaantal \\[2mm]
        
        Documentatie‑Kwaliteit &
        gedetecteerde grammaticale fouten &
        Code Spellchecker\footnote{\href{https://marketplace.visualstudio.com/items?itemName=streetsidesoftware.code-spell-checker}{https://marketplace.visualstudio.com/items?itemName=streetsidesoftware.code-spell-checker}} / MS Word &
        $\leq2$ spelfouten per $100$ woorden \\[2mm]
        
        Kosten &
        kosten per run &
        token‑gebruik meten met tiktoken\footnote{\href{https://github.com/openai/tiktoken}{https://github.com/openai/tiktoken}} &
        $\leq€0.10$ per test‑case \\
        \bottomrule
        \addlinespace[1mm]
        \multicolumn{4}{l}{*~Nog niet 100\% concreet, voorbeeld doelwaarden.} \\[2mm]
        \multicolumn{4}{l}{**~Er wordt bij IT1 met PHP + Laravel gewerkt; PHPUnit en Pest zijn geschikte test‑frameworks voor die code‑omgeving. \autocite{Laravel}} \\
    \end{tabular}
\end{sidewaystable}
...
\begin{tikzpicture}
    ...
    {font=\small\bfseries},
    AI Agent/.style     = {concept, ball color=blue,
        font=\Huge\bfseries},
    ...
    node [AI Agent] {AI Agent} [clockwise from=0]
    ...
    child [concept color=red, nodes={LLMs}] {
        ...
        child { node {Modellen} [clockwise from=300]
            child { node {X vs Y} }}
        child { node {Prompts} [clockwise from=90]
            child { node {Tokens} }}
        child { node {Ollama} [clockwise from=330]}}
    child [concept color=orange, nodes={OpenAPI}] {
        node {OpenAPI} };
\end{tikzpicture}

Hier zien we een uitgezoomde kijk op de verschillende aspecten binnen dit onderzoek. Code voor het genereren van de mindmap\footnote{\href{https://latex-cookbook.net/mindmap/}{https://latex-cookbook.net/mindmap/}} verkregen van: \autocite{Kottwitz2024}

%in/out comment for BP/voorstel
%\twocolumn
